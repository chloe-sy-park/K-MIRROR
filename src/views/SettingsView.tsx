...
const SettingsView = ({ isSensitive, onToggleSensitive, onResetData }: SettingsViewProps) => {
  return (
    <motion.div key="settings" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto space-y-16 py-12 px-6">
      ...
      <button onClick={onResetData} className="w-full py-6 text-[10px] font-black uppercase tracking-widest text-gray-200 hover:text-red-500 transition-colors uppercase uppercase">Reset Neural Stylist Data</button>
    </motion.div>
  );
};
