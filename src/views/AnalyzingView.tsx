...
const AnalyzingView = () => {
  ...
  return (
    <m.div key="analyzing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-[60vh] flex flex-col items-center justify-center gap-16">
      <div className="relative w-80 md:w-[26rem] aspect-[3/4] bg-gray-50 rounded-[4rem] overflow-hidden scanning shadow-2xl border border-gray-100">
        {userImage && <m.img initial={{ scale: 1.2 }} animate={{ scale: 1 }} transition={{ duration: 10 }} src={`data:image/jpeg;base64,${userImage}`} className="w-full h-full object-cover opacity-50 grayscale" alt="Scanning User" />}
